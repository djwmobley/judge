...
if (require.main === module) {
  ...
  try {
    ...
    // file as event "guard_crash". The real appendCrashRecord already
    // swallows its own errors, but this call site wraps it again anyway:
    // a corrupted/replaced decisions module (install drift) might not
    // honor that contract, and this is the last line of defense before
    // process.exit — nothing downstream can catch a throw here. Either
    // way the guard's own exit code (2, unchanged from before this spec)
    // is decided independently below, never by whether this call throws.
    try {
