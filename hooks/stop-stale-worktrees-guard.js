...
// ─── Tamper evidence (docs/specs/hook-state-write-guard.md §3) ────────────
//
// A separate HMAC-SHA256 keyfile (`<state dir>/.hmac-key`, exactly 32
// random bytes) signs each state file's `items` field, bound to the
// session key the state filename itself is derived from (closes
// cross-session replay -- spec §3.2/A5). Every failure mode here (keyfile
// missing/unreadable/wrong-length, mac missing, mac mismatch) fails
// *closed*: the state is treated as absent for this invocation, which can
// only ever DELAY a yield, never grant one -- the same safe-direction
// principle §3 step 2 already applies to ordinary corrupt-JSON handling.

const HMAC_KEY_FILENAME = ".hmac-key";
const HMAC_KEY_BYTES = 32;

function hmacKeyPath(stateDir) {
  return path.join(stateDir, HMAC_KEY_FILENAME);
}

/**
 * Returns a 32-byte Buffer key, or `null` if unavailable for any reason
 * (read failure, create failure, or existing content whose length is not
 * exactly 32 bytes -- spec §3.1/A4). A wrong-length keyfile is NEVER
 * regenerated automatically: it may simply be mid-write by a concurrent
 * process, and silently overwriting another session's in-use key is a
 * worse outcome than this invocation temporarily failing closed.
 *
 * Create-if-absent uses the same tmp-file + renameSync atomic-write
 * convention as `writeReblockStateAtomic` below. A benign race between two
 * concurrent first-run processes is resolved by whichever `renameSync`
 * wins; the loser's generated bytes are simply discarded and the final
 * read below picks up whichever key actually landed on disk.
 */
function readOrCreateHmacKey(fsx, stateDir) {
  const p = hmacKeyPath(stateDir);
  let existing = null;
  try {
    existing = fsx.readFileSync(p);
  } catch (_) {
    existing = null; // missing or unreadable -- fall through to create-if-absent.
  }
  if (existing !== null && existing !== undefined) {
    const buf = Buffer.isBuffer(existing) ? existing : Buffer.from(existing, "binary");
    if (buf.length === HMAC_KEY_BYTES) return buf;
    return null; // present but wrong length -- unavailable, fail closed (A4).
  }
  try {
    const fresh = crypto.randomBytes(HMAC_KEY_BYTES);
    fsx.mkdirSync(stateDir, { recursive: true });
    const tmpPath = `${p}.tmp.${process.pid}.${Date.now()}.${Math.random().toString(36).slice(2)}`;
    fsx.writeFileSync(tmpPath, fresh, { mode: 0o600 });
    fsx.renameSync(tmpPath, p);
  } catch (_) {
    // Fall through to the read below -- a concurrent process may have
    // already created the file (benign create race, spec §3.1).
  }
  try {
    const final = fsx.readFileSync(p);
    const buf = Buffer.isBuffer(final) ? final : Buffer.from(final, "binary");
    if (buf.length === HMAC_KEY_BYTES) return buf;
    return null;
  } catch (_) {
    return null;
...
/**
 * Canonical JSON of `items`, per spec §3.2: keys of `items` itself sorted,
 * and each item value object's own keys sorted, then `JSON.stringify`
 * (whose default output already has no inter-token whitespace). Non-object
 * `items` is treated as `{}` -- callers here always pass a real items
 * object, but this keeps the function total.
 */
function canonicalizeItems(items) {
  ...
  for (const key of Object.keys(src).sort()) {
    ...
    if (val && typeof val === "object" && !Array.isArray(val)) {
      const innerCanonical = {};
      for (const innerKey of Object.keys(val).sort()) innerCanonical[innerKey] = val[innerKey];
      canonical[key] = innerCanonical;
    } else {
      canonical[key] = val;
    }
  }
 ...
 * one). This is NOT a tamper event -- the ordinary first-run/corrupt-JSON
 * case, unchanged from the pre-tamper-evidence behavior.
 *
...
function readReblockState(fsx, statePath, verifyCtx) {
  ...
  try {
    ...
    if (parsed && typeof parsed === "object" && parsed.items && typeof parsed.items === "object" && !Array.isArray(parsed.items)) {
      obj = parsed;
    } else {
      return { items: {} };
    }
...
function applyBoundedReblock(baseResult, stdinInfo, deps) {
  ...
  // Tamper evidence (docs/specs/hook-state-write-guard.md §3): obtain the
  // HMAC keyfile BEFORE reading state, so the read can verify the stored
  // `mac` against the session key that names this state file. Keyfile
  // unavailable for any reason (missing/unreadable/wrong-length, §3.1/A4)
  // -> `keyBytes` is `null` -> `readReblockState` cannot verify and treats
  // the state as absent, identically to a MAC mismatch (fail closed).
